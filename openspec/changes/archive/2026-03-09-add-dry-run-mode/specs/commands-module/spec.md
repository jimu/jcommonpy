...
- **WHEN** `jcli.command.execute("echo 'test'", "json", dry_run=True)` is called
- **THEN** prints the command to stdout and returns a dry-run result without executing

#### Scenario: Execute command in verbose mode
- **WHEN** `jcli.command.execute("echo 'test'", "json", verbose=True)` is called
- **THEN** prints the command to stdout and executes it normally
